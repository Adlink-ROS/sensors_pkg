...
private:
    ...
    message_filters::Subscriber<sensor_msgs::msg::Imu> _imu_sub;
    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::msg::Image, sensor_msgs::msg::PointCloud2, sensor_msgs::msg::Imu> approximate_policy;
...
public:
                          ...
                          _syncApproximate(_approximate_policy)
    {
        ...
        _imu_sub.subscribe(this, "imu/data");
        _syncApproximate.connectInput(_image_sub, _lidar_sub, _imu_sub);
        _syncApproximate.registerCallback(
            bind(&SensorsSubscriber::callback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    }
};
...
int main(int argc, char ** argv)
{
    ...
    cout << "Start to subscribe lidar, camera and imu topic" << endl;
